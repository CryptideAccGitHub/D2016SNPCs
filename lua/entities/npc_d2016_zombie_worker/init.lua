...
--Basic set-up
ENT.ModelTable = {"models/monsters/possessed/possessed_worker.mdl"}
...
ENT.GoreBones = {"head","spine3","leftleg","leftupleg","rightleg","rightupleg","rightarm","rightforearm"}
	...
	if self:CanPerformProcess() and self:GetNoDraw() == false then
		...
		if dist < 150 and self:FindInCone(enemy,90) and math.random(1,3) == 1 then
			if self:IsMoving() then
				...
				self:PlayActivity(self:SelectFromTable({"melee_special_uacsecurity_moving"}))
			else
				sound.Play("doom2016/possessed/unwillingattack"..math.random(1,3)..".ogg",self:GetPos())
				self:PlayActivity(self:SelectFromTable({"melee_special_uacsecurity_moving"}))
			end
		elseif dist < 100 and self:FindInCone(enemy,90) and not self:IsMoving() then
			sound.Play("doom2016/possessed/unwillingattack"..math.random(1,3)..".ogg",self:GetPos())
			self:PlayActivity(self:SelectFromTable({"melee_lunge_short_right_arm"}))
			return
		elseif dist < 150 and self:FindInCone(enemy,60) and self:IsMoving() then
			sound.Play("doom2016/possessed/unwillingattack"..math.random(1,3)..".ogg",self:GetPos())
			self:PlayActivity(self:SelectFromTable({"melee_moving_fwd_lunge_right_arm"}))
			return
		end
